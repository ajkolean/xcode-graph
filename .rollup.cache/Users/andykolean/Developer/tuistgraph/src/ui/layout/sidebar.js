...
const lit_template_1 = { h: b_1 `
    ...
    `, parts: [{ type: 1, index: 1, name: "click", strings: ["", ""], ctor: E_1 }, { type: 1, index: 2, name: "is-collapsed", strings: ["", ""], ctor: B_1 }, { type: 2, index: 5 }] };
const lit_template_2 = { h: b_1 `
              ...
              `, parts: [{ type: 1, index: 0, name: "class", strings: ["", ""], ctor: A_1 }, { type: 1, index: 0, name: "aria-current", strings: ["", ""], ctor: A_1 }, { type: 1, index: 0, name: "click", strings: ["", ""], ctor: E_1 }, { type: 2, index: 3 }, { type: 2, index: 5 }, { type: 2, index: 6 }] };
const lit_template_3 = { h: b_1 `<span class="dropdown-icon"><?></span>`, parts: [{ type: 2, index: 1 }] };
/**
 * Left sidebar navigation with staggered entrance animations.
 * Features noise texture, accent borders, and monospace typography.
 *
...
export class GraphSidebar extends LitElement {
  ...
  `;
    ...
    // ========================================
    render() {
        return { ["_$litType$"]: lit_template_1, values: [this.toggleCollapse, this.collapsed, repeat(this.navItems, (item) => item.id, (item) => {
                    const isActive = this.activeTab === item.id;
                    return { ["_$litType$"]: lit_template_2, values: [classMap({ 'nav-button': true, active: isActive }), isActive ? 'page' : nothing, () => this.handleTabClick(item.id), unsafeHTML(item.iconSvg), item.label, when(item.hasDropdown, () => ({ ["_$litType$"]: lit_template_3, values: [unsafeHTML(icons.ChevronDown)] }))] };
                })] };
    }
}
__decorate([
    property({ type: Boolean, reflect: true })
], GraphSidebar.prototype, "collapsed", void 0);
__decorate([
    property({ type: Boolean })
], GraphSidebar.prototype, "defaultCollapsed", void 0);
__decorate([
    property({ type: String, attribute: 'active-tab' })
], GraphSidebar.prototype, "activeTab", void 0);
