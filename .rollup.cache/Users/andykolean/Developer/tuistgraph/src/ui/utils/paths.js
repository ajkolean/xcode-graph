 ...
 */
function getPathCacheKey(x1, y1, x2, y2) {
 ...
 */
export function generateWaypointPath(start, waypoints, end) {
 ...
 * @returns SVG path string
 */
export function generatePortRoutedPath(sourceNode, sourcePort, targetPort, targetNode, waypoints, sourceClusterCenter, targetClusterCenter, options = {}) {
    ...
    parts.push(`C ${sourceWorld.x + exitOffset},${sourceWorld.y} ${sourcePort.x - exitOffset * Math.sign(exitDx)},${sourcePort.y} ${sourcePort.x},${sourcePort.y}`);
    ...
    parts.push(`C ${targetPort.x + entryOffset * Math.sign(entryDx)},${targetPort.y} ${targetWorld.x - entryOffset},${targetWorld.y} ${targetWorld.x},${targetWorld.y}`);
    return parts.join(' ');
}
