...
import { calculateViewportBounds, cullEdges, cullNodes, estimateCullingRatio, isCircleInViewport, isLineInViewport, isPointInViewport, } from './viewport';
describe('calculateViewportBounds', () => {
    ...
    it('should include margin', () => {
        ...
        expect(withMargin.maxX - withMargin.minX).toBeGreaterThan(withoutMargin.maxX - withoutMargin.minX);
    });
});
...
describe('isLineInViewport', () => {
    ...
    it('should return true if one endpoint is inside', () => {
        const start = { x: 100, y: 100 };
        const end = { x: 1500, y: 1000 }; // Outside
        expect(isLineInViewport(start, end, bounds)).toBe(true);
    });
    it('should return true if line passes through viewport', () => {
        const start = { x: -100, y: 400 }; // Left of viewport
        const end = { x: 1100, y: 400 }; // Right of viewport
        expect(isLineInViewport(start, end, bounds)).toBe(true);
    });
    ...
    it('should return false for lines parallel to viewport but outside', () => {
        const start = { x: 1100, y: 100 }; // Right of viewport
        const end = { x: 1100, y: 700 };
        expect(isLineInViewport(start, end, bounds)).toBe(false);
    });
