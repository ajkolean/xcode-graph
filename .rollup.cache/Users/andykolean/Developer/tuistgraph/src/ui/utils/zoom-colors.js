 ...
 */
import { normalizeZoom, ZOOM_LIGHTNESS_ADJUSTMENT, ZOOM_OPACITY, ZOOM_SATURATION, ZOOM_STROKE_WIDTH, } from '@shared/utils/zoom-constants';
 ...
 */
function getSaturationMultiplier(zoom) {
 ...
 */
function getLightnessAdjustment(zoom) {
 ...
 */
export function adjustColorForZoom(color, zoom) {
 ...
 */
export function adjustOpacityForZoom(baseOpacity, zoom) {
    const t = normalizeZoom(zoom);
    const opacityMultiplier = ZOOM_OPACITY.MIN_MULTIPLIER + (ZOOM_OPACITY.MAX_MULTIPLIER - ZOOM_OPACITY.MIN_MULTIPLIER) * t;
...
//# sourceMappingURL=zoom-colors.js.map