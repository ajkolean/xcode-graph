...
describe('xcode-graph-cluster-details-panel - Event Bubbling', () => {
    ...
    `);
        const header = el.shadowRoot?.querySelector('xcode-graph-cluster-header');
        setTimeout(() => header?.dispatchEvent(new CustomEvent('back', { bubbles: true, composed: true })));
    ...
    `);
        const targetsList = el.shadowRoot?.querySelector('xcode-graph-cluster-targets-list');
        setTimeout(() => targetsList?.dispatchEvent(new CustomEvent('node-select', {
    ...
    `);
        const targetsList = el.shadowRoot?.querySelector('xcode-graph-cluster-targets-list');
        setTimeout(() => targetsList?.dispatchEvent(new CustomEvent('node-hover', {
            detail: { nodeId: 'node1' },
            bubbles: true,
            composed: true,
        })));
...
//# sourceMappingURL=cluster-details-panel.test.js.map