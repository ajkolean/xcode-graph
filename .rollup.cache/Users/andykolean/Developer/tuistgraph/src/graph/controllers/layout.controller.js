...
export class LayoutController {
     ...
     */
    async computeLayout(nodes, edges, forceRecompute = false) {
        ...
        try {
            ...
            const { clusterPositions: initialClusterPos, nodePositions: initialNodePos, clusters: layoutClusters, cycleNodes, nodeSccId, sccSizes, clusterEdges, routedEdges, } = await computeHierarchicalLayout(nodes, edges, analyzedClusters);
    ...
    // Private Helpers
    // ========================================
    isSameInput(nodes, edges) {
        return (nodes.length === this.cachedNodes.length &&
            edges.length === this.cachedEdges.length &&
            nodes === this.cachedNodes &&
            edges === this.cachedEdges);
    }
    cacheResult(result, nodes, edges) {
