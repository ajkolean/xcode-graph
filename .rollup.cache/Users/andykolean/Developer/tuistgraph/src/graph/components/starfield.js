...
export class Starfield {
    ...
    height = 0;
    options;
    palette = DEFAULT_STAR_PALETTE;
    // Cached bitmap for static starfield
    cachedCanvas = null;
    cachedPanX = Number.NaN;
    cachedPanY = Number.NaN;
    constructor(options = {}) {
     ...
     */
    needsRegeneration(width, height) {
        return (width > 0 &&
            height > 0 &&
    ...
    createStar(isBright, spanX, spanY, offsetX, offsetY) {
     ...
     */
    render(ctx, panX, panY) {
        const stars = this.stars;
        const count = stars.length;
        if (count === 0 || this.width <= 0 || this.height <= 0)
            return;
