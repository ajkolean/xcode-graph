...
function isClusterInViewport(cx, cy, radius, viewport) {
    return !(cx + radius < viewport.minX ||
...
function getClusterFillOpacity(nodeCount, isActive) {
    if (isActive)
        return 0.08;
    if (nodeCount <= 5)
        return 0.06;
...
function getClusterBorderWidth(nodeCount, isActive) {
    const baseWidth = isActive ? 2.5 : 2;
    return Math.max(1, Math.log2(nodeCount)) * baseWidth * 1.5;
}
function drawClusterFillAndBorder(ctx, cx, cy, radius, clusterColor, isActive, isSelected, shouldDim, clusterType, zoom, time, nodeCount) {
...
function getAdaptiveClusterFontSize(targetScreenSize, zoom) {
...
function drawClusterLabels(ctx, cx, cy, radius, clusterColor, isActive, shouldDim, name, zoom) {
    const dimFactor = shouldDim ? 0.3 : 1.0;
...
function drawCentroidDot(ctx, cx, cy, clusterColor, nodeCount, shouldDim, alpha) {
...
function getZoomCrossfade(zoom) {
    if (zoom < 0.15)
        return { centroidAlpha: 1.0, fullAlpha: 0 };
    if (zoom < 0.3) {
...
function renderSingleCluster(rc, cx, cy, radius, clusterColor, isActive, isSelected, shouldDim, clusterType, nodeCount, centroidAlpha, fullAlpha) {
    ...
    if (fullAlpha > 0) {
        ...
        drawClusterFillAndBorder(ctx, cx, cy, radius, clusterColor, isActive, isSelected, shouldDim, clusterType, zoom, time, nodeCount);
    }
}
export function renderClusters(rc, viewport) {
    ...
    for (const cluster of layout.clusters) {
        ...
        if (!isClusterInViewport(cx, cy, radius, viewport))
            continue;
        ...
        renderSingleCluster(rc, cx, cy, radius, clusterColor, isActive, selectedCluster === cluster.id, shouldDim, cluster.type, cluster.nodes.length, centroidAlpha, fullAlpha);
...
//# sourceMappingURL=canvas-cluster-renderer.js.map