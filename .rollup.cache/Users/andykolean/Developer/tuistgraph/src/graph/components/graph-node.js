...
export class GraphNode extends LitElement {
    ...
    // ========================================
    handleMouseEnter() {
        this.dispatchEvent(new CustomEvent('node-mouseenter', {
            bubbles: true,
            composed: true,
        }));
    }
    handleMouseLeave() {
        this.dispatchEvent(new CustomEvent('node-mouseleave', {
            bubbles: true,
            composed: true,
        }));
    }
    ...
    getShowTooltip(nodeName, isHovered) {
        return isHovered && nodeName.length > 20;
    }
    // ========================================
    // Render Helpers
    // ========================================
    renderSonarPulses(x, y, size, color) {
        const pulseConfigs = [
    ...
    renderGlowRings(x, y, size, color, isSelected, glowOpacity) {
        return svg `
      <circle
        cx="${x}"
        cy="${y}"
    ...
    renderLabel(options) {
        ...
        const textFill = isSelected || isHovered ? color : 'rgba(var(--colors-foreground-rgb), var(--opacity-90))';
        const getFontWeight = () => {
            if (isSelected)
                return '600';
            if (isHovered)
    ...
    getNodeScale(isHovered, isSelected) {
        if (isHovered)
            return 1.15;
        if (isSelected)
            return 1.08;
    ...
    getIconStrokeWidth(isSelected, isHovered) {
        if (isSelected)
            return 2.5;
        if (isHovered)
            return 2.2;
    ...
    renderBackgroundCircle(x, y, size) {
        return svg `
      <circle
        cx="${x}"
        cy="${y}"
    ...
    renderIconShape(x, y, iconPath, color, isSelected, isHovered, showGlow) {
        return svg `
      <g
    ...
    render() {
        ...
        const icon = this.renderIconShape(x, y, iconPath, zoomAdjustedColor, isSelected, isHovered, showGlow);
        const label = zoom >= 0.5
            ? this.renderLabel({
                position: { x, y },
                size,
