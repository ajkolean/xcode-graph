 ...
 */
export function computePortSide(sourceX, sourceY, targetX, targetY) {
 ...
 */
function initializePortRequirements(clusterPositions) {
    const portRequirements = new Map();
    for (const clusterId of clusterPositions.keys()) {
        portRequirements.set(clusterId, new Map([
            ['NORTH', []],
            ['SOUTH', []],
            ['EAST', []],
            ['WEST', []],
...
function collectPortRequirements(clusterEdges, clusterPositions, portRequirements) {
...
export function computeClusterPorts(clusterPositions, clusterEdges, config) {
    ...
    for (const [clusterId, sideReqs] of portRequirements) {
        ...
        for (const [side, requirements] of sideReqs) {
            ...
            const ports = computePortPositionsOnSide(clusterId, side, clusterPos, limitedReqs.length, config);
 ...
 */
function computePortPositionsOnSide(clusterId, side, clusterPos, portCount, config) {
    const ports = [];
    if (portCount === 0)
        return ports;
 ...
 */
function assignClusterEdgesToPorts(clusterEdges, clusterPorts, clusterPositions) {
 ...
 */
function findBestPort(ports, side, targetX, targetY) {
 ...
 */
export function computeRoutedEdges(edges, nodePositions, clusterPositions, clusterPorts, clusterEdges, nodeToCluster, config) {
    if (!config.portRoutingEnabled) {
        return [];
    }
    const routedEdges = [];
    // First, get cluster-level port assignments
    const clusterPortAssignments = assignClusterEdgesToPorts(clusterEdges, clusterPorts, clusterPositions);
    ...
    for (const edge of edges) {
        ...
        const waypoints = computeWaypoints(assignment.sourcePort, assignment.targetPort, clusterPositions.get(sourceClusterId), clusterPositions.get(targetClusterId));
        routedEdges.push({
 ...
 */
function computeWaypoints(sourcePort, targetPort, _sourceCluster, _targetCluster) {
    const waypoints = [];
    ...
    else if ((sourcePort.side === 'NORTH' || sourcePort.side === 'SOUTH') &&
...
//# sourceMappingURL=port-routing.js.map