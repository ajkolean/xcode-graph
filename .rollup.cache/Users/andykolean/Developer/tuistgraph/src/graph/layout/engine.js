...
export async function computeHierarchicalLayout(nodes, edges, clusters, opts = {}) {
    ...
    // (Can run in parallel)
    const microLayouts = new Map(clusters.map((cluster) => {
    ...
    if (config.portRoutingEnabled) {
        ...
        routedEdges = computeRoutedEdges(edges, nodePositions, clusterPositions, clusterPorts, clusterGraph.edges, clusterGraph.nodeToCluster, config);
    }
    return {
        nodePositions,
        clusterPositions,
