...
function checkCircularOverlap(c1, c2) {
...
function findOverlappingPairs(positions) {
    const failures = [];
    for (let i = 0; i < positions.length; i++) {
        for (let j = i + 1; j < positions.length; j++) {
            ...
            if (isOverlap) {
                failures.push(`${c1.id} vs ${c2.id} (Dist: ${dist.toFixed(1)}, Required: ${minSpace.toFixed(1)})`);
            }
        }
    }
    return failures;
...
//# sourceMappingURL=overlap.test.js.map