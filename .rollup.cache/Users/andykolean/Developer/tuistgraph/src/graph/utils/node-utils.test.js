...
import { createDiamondGraph, createLinearChain, createNode, createNodesForFilterTesting, createProjectGraph, } from '../../fixtures';
...
describe('computeNodeDependencies', () => {
    ...
    it('should include edge info with each dependency', () => {
        ...
        const nodeA = nodes.find((n) => n.id === 'A');
        if (!nodeA)
            throw new Error('Node A not found');
        const result = computeNodeDependencies(nodeA, nodes, edges);
    ...
    it('should compute total counts from unfiltered edges', () => {
        ...
        const filteredEdges = [{ source: 'A', target: 'B' }];
    ...
    it('should compute totalConnections as sum of deps + dependents', () => {
        ...
        expect(result.metrics.totalConnections).toBe(result.metrics.dependencyCount + result.metrics.dependentCount);
    });
});
...
//# sourceMappingURL=node-utils.test.js.map