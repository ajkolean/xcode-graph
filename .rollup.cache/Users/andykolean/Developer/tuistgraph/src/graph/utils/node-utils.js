 ...
 * ```
 */
export function computeNodeDependencies(node, allNodes, edges, filteredEdges) {
    if (!node) {
        return {
            dependencies: [],
            dependents: [],
    ...
    const totalDependencyCount = edges.filter((e) => e.source === node.id && nodeMap.has(e.target)).length;
    const totalDependentCount = edges.filter((e) => e.target === node.id && nodeMap.has(e.source)).length;
    // Transitive counts using shared traversal utility
    const { outgoing, incoming } = buildAdjacency(edges);
    const depsResult = traverseGraph(node.id, (id) => outgoing.get(id) ?? [], (current, neighbor) => `${current}->${neighbor}`);
    // Subtract 1 to exclude the start node itself
    const transitiveDependencyCount = depsResult.nodes.size - 1;
    const dependentsResult = traverseGraph(node.id, (id) => incoming.get(id) ?? [], (current, neighbor) => `${neighbor}->${current}`);
 ...
 */
export function computeClusterStats(clusterNodes, edges, filteredEdges) {
    const edgesToUse = filteredEdges || edges;
    // Filtered stats
    ...
    const filteredClusterNodeIds = new Set(filteredEdges
        ? clusterNodes
 ...
 */
export function computeFilters(allNodes) {
    ...
    const hasActiveFilters = (filters) => {
        return (filters.nodeTypes.size < typeCounts.size ||
    ...
    const createClearFilters = (onFiltersChange) => {
        return () => {
            onFiltersChange({
