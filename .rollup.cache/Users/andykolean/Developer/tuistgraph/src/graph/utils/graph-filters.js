...
function matchesFilterCriteria(node, filters) {
    ...
    if (node.deploymentTargets) {
        ...
        if (!nodePlatforms.some((p) => filters.platforms.has(p)))
            return false;
    }
    else {
        if (!filters.platforms.has(node.platform))
            return false;
    }
    if (!filters.origins.has(node.origin))
        return false;
    ...
    if (node.type === NodeType.Package && !filters.packages.has(node.name))
        return false;
    return true;
}
/**
 ...
 * ```
 */
export function applyGraphFilters(nodes, edges, filters, searchQuery) {
    const normalizedQuery = searchQuery.trim().toLowerCase();
    const filteredNodes = nodes.filter((node) => matchesFilterCriteria(node, filters) && matchesSearch(node, normalizedQuery));
