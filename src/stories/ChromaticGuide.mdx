...
<Meta title="Introduction/Chromatic Guide" />

# Chromatic Features Guide

<div style={{ padding: '2rem', background: 'linear-gradient(135deg, rgba(252, 126, 126, 0.1), rgba(249, 177, 21, 0.1))', borderRadius: '8px', marginBottom: '2rem' }}>
  <p style={{ fontSize: '1.1rem', marginBottom: 0 }}>
    This guide explains the Chromatic features available in this project and how to use them for
    visual regression testing and accessibility testing in your CI/CD pipeline.
  </p>
</div>
...
**Visual Regression Tests** - Running in CI on every PR
...
Chromatic automatically captures screenshots of all your stories and compares them against baseline images. This helps catch unintended visual changes before they reach production.

**Key Benefits**:
- **Automated visual QA** - No manual screenshot comparison needed
- **TurboSnap** - Only tests changed components for faster builds
- **Multi-mode testing** - Tests light/dark themes and multiple viewports
- **PR integration** - Visual diffs shown directly in pull requests

### How It Works

1. **Baseline Creation**: First build establishes baseline screenshots
2. **Change Detection**: Subsequent builds compare against baseline
3. **Review Process**: Team reviews diffs in Chromatic UI
4. **Baseline Updates**: Approved changes become new baseline
...
3. CI automatically runs Chromatic build
4. Review visual changes in Chromatic dashboard

**During Code Review**:
1. PR shows Chromatic status check
2. Click "View changes" to see visual diffs
3. Approve intentional changes or request fixes
4. Merge when all checks pass

### Configuration

Our Chromatic setup tests multiple scenarios:

```typescript
// Configured in .storybook/preview.ts
chromatic: {
  delay: 1000, // Wait for Lit components to register
  pauseAnimationAtEnd: true, // Consistent snapshots
  modes: {
    light_desktop: { backgrounds: 'light', viewport: 'desktop' },
    dark_desktop: { backgrounds: 'dark', viewport: 'desktop' },
    light_mobile: { backgrounds: 'light', viewport: 'mobile' },
    dark_mobile: { backgrounds: 'dark', viewport: 'mobile' },
  },
}
```

This configuration ensures we test:
- ✅ Light and dark color schemes
- ✅ Desktop and mobile viewports
- ✅ All Lit components are fully rendered
...
Chromatic automatically runs accessibility tests on all your stories in CI and tracks violations over time using regression-based testing.

**How It Differs from Local Testing**:
...
The first time you review accessibility results, you'll need to establish a baseline. This tells Chromatic "these are the violations we're aware of."

#### Step 1: Check Current Status

1. Go to your Chromatic project dashboard
2. Navigate to the "Accessibility" tab
3. Review all violations detected in your latest build

#### Step 2: Choose Your Approach

**Option A: Accept Current State as Baseline** (Recommended for starting)
- Allows you to prevent NEW violations without blocking on existing ones
- Fix legacy violations incrementally over time
- Future PRs only fail if they introduce NEW violations

**Option B: Fix All Violations First**
...
To fail builds on new accessibility violations, update `.github/workflows/chromatic.yml`:
...
- name: Publish to Chromatic
  ...
  with:
    ...
    exitZeroOnChanges: false  # Change from true to fail on violations
    exitOnceUploaded: true
```

This will cause CI to fail if:
...
You can customize which rules are checked globally or per-story:
...
export const MyStory: Story = {
  parameters: {
    a11y: {
      disable: true, // Disable all a11y tests for this story
    },
  },
};
```
...
## Visual Tests Addon (Local Development)
...
Or globally in `.storybook/preview.ts` as shown in the Accessibility Testing section.
...
Our `.github/workflows/chromatic.yml` is configured to:

1. **Install dependencies** - Set up pnpm and install packages
2. **Install Playwright** - Required for test-storybook
3. **Build Storybook** - Create static build
4. **Run interaction tests** - Execute play functions via test-storybook
5. **Publish to Chromatic** - Upload to Chromatic for visual testing

This ensures:
- ✅ All interaction tests pass before visual testing
- ✅ Visual regression tests run on every PR
- ✅ Accessibility tests run automatically
- ✅ Fast feedback with TurboSnap

### Monitoring Test Results

**In Pull Requests**:
- GitHub shows Chromatic status check
- Click "Details" to view Chromatic build
- Review visual changes and accessibility results

**In Chromatic Dashboard**:
- View all test runs
- Track accessibility violation trends
- Manage baselines
...
<div style={{ padding: '1.5rem', background: 'rgba(249, 177, 21, 0.1)', borderRadius: '8px', marginTop: '2rem' }}>
  <strong>Ready to get started?</strong>
  <p style={{ marginBottom: 0, marginTop: '0.5rem' }}>
    Check out our <a href="/story/foundations-accessibility--keyboard-navigation">Accessibility Guide</a> to see
    accessibility testing in action, or visit the <a href="/story/introduction-testing-guide">Testing Guide</a> for
    more information on our comprehensive testing strategy.
  </p>
</div>
