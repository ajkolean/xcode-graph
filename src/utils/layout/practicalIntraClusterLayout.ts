...
import { Cluster, PositionedNode, ClusterLayoutConfig, NodeRole } from '../../types/cluster';
...
export function layoutNodesInClusterPractical(
  ...
  for (const ring of [1, 2]) {
    ...
    const scored = ringNodes.map(node => {
      ...
      const edgeCount = internalEdges.filter(e => 
  ...
  testNodes.forEach(testNode => {
    ...
    if (subjects.length === 0) {
      ...
      if (subjectPos) {
        ...
        positioned.push(createPositionedNode(testNode, cluster, x, y, 
