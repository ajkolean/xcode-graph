...
export function layoutNodesOptimal(
  ...
  for (const [_ring, ringNodes] of nodesByRing.entries()) {
 ...
 */
function computeSpectralOrdering(nodes: GraphNode[], edges: GraphEdge[]): GraphNode[] {
  ...
  const degrees = nodes.map((_, i) => adjacency[i].reduce((sum, val) => sum + val, 0));
