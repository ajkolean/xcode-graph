...
export function layoutItemsInRings(
  ...
  const actualAnchors = anchors.length > 0 ? anchors : [findMostConnectedNode(nodes, edges)];
 ...
 */
function buildReverseAdjacency(nodes: LayoutNode[], edges: LayoutEdge[]): Map<string, Set<string>> {
 ...
 */
function groupByRing(nodes: LayoutNode[], rings: Map<string, number>): Map<number, LayoutNode[]> {
 ...
 */
function findMostConnectedNode(nodes: LayoutNode[], edges: LayoutEdge[]): LayoutNode {
  const nodeIds = new Set(nodes.map((n) => n.id));
