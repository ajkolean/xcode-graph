...
import { computeClusterLayout, ClusterLayoutResult } from './clusterLayout';
import { simpleClusterLayout, computeMEC } from './simpleClusterLayout';
...
export function computeHierarchicalLayout(
  ...
  for (const cluster of clusters) {
    ...
    console.log(`  ✓ Cluster ${cluster.id}: ${clusterNodes.length} nodes → ${Math.round(dimension)}px (mecRadius: ${Math.round(mecRadius)})`);
    
    clusterDimensions.set(cluster.id, dimension);
  }

  console.log(`📦 Total dimensions computed: ${clusterDimensions.size}`);
  console.log('📊 Dimensions map:', Array.from(clusterDimensions.entries()).map(([id, dim]) => `${id}: ${Math.round(dim)}px`));
