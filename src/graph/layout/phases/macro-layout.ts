...
export async function computeMacroLayout(
  ...
  const edges: ElkExtendedEdge[] = clusterGraph.edges.map((e) => ({
    ...
    layoutOptions: {
      ...
      "elk.layered.priority.direction": String(
        Math.min(10, 1 + Math.log2(e.weight ?? 1)),
      ),
  ...
  const totalArea = children.reduce(
    (sum, c) => sum + (c.width ?? 0) * (c.height ?? 0),
    0,
  );
  ...
  if (layout.children) {
    ...
    for (const [_, nodes] of sortedBands) {
      ...
      for (const row of rows) {
        ...
        const maxRowH = row.reduce(
          (max, n) => Math.max(max, n.height ?? 100),
          0,
        );
