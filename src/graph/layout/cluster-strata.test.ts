...
describe("seedClustersByStrata", () => {
  it("should position clusters in horizontal bands by stratum", () => {
    ...
    const positions = seedClustersByStrata(
      clusters,
      strataResult,
      clusterSizes,
      config,
...
describe("Determinism", () => {
  it("should produce identical results for same inputs", () => {
    ...
    for (const id of clusterIds) {
      expect(result1.clusterStratum.get(id)).toBe(
        result2.clusterStratum.get(id),
      );
    }
  ...
  it("should produce consistent positions across multiple runs", () => {
    ...
    const positions1 = seedClustersByStrata(
      clusters,
      strataResult,
      clusterSizes,
      config,
    );
    const positions2 = seedClustersByStrata(
      clusters,
      strataResult,
      clusterSizes,
      config,
