 ...
 */
async function renderCluster(props: {
	clusterId?: string;
	x?: number;
	y?: number;
	width?: number;
...
describe('xcode-graph-cluster', () => {
	it('should render the cluster label', async () => {
		...
		const labelText = Array.from(texts).find((t) =>
			t.textContent?.trim() === 'CoreProject',
		);
	...
	it('should render the target count and origin text', async () => {
		...
		const texts = el.querySelectorAll('text');
		const infoText = Array.from(texts).find((t) =>
			t.textContent?.trim().includes('LOCAL') && t.textContent?.trim().includes('7 targets'),
		);
	...
	it('should show EXTERNAL for external origin', async () => {
		...
		const texts = el.querySelectorAll('text');
		const infoText = Array.from(texts).find((t) =>
			t.textContent?.trim().includes('EXTERNAL'),
		);
		expect(infoText).to.exist;
	});
	...
	it('should show LOCAL for local origin', async () => {
		...
		const texts = el.querySelectorAll('text');
		const infoText = Array.from(texts).find((t) =>
			t.textContent?.trim().includes('LOCAL'),
		);
		expect(infoText).to.exist;
	});
