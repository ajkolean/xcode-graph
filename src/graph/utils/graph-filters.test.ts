...
describe('matchesSearch (fuzzy)', () => {
  ...
  it('should match by node name', () => {
    expect(matchesSearch(node, 'HomeFeature')).toBe(true);
  });

  it('should fuzzy match by node name', () => {
    // Fuzzy: "HomeFeat" is close enough to "HomeFeature"
    expect(matchesSearch(node, 'HomeFeat')).toBe(true);
  });
  ...
  it('should handle node without project', () => {
    ...
    expect(matchesSearch(nodeWithoutProject, 'zzzzz')).toBe(false);
  });
});
