...
import { transformTuistGraph } from './tuist-graph.service';
import { safeParseGraph } from './tuist-graph.validation';

const FIXTURE_PATH = resolve(__dirname, '../fixtures/tuist-graph.json');
const FUTURE_FIXTURE_PATH = resolve(__dirname, '../fixtures/tuist-graph-future.json');
...
describe('forward-compatibility: missing or empty sections', () => {
  ...
  it('should fail gracefully for completely invalid input', () => {
    const result = transformTuistGraph('not an object');
    expect(result.data.nodes).toEqual([]);
    expect(result.data.edges).toEqual([]);
  ...
  it('should fail gracefully for null input', () => {
    const result = transformTuistGraph(null);
    expect(result.data.nodes).toEqual([]);
    expect(result.data.edges).toEqual([]);
  ...
  it('should fail gracefully for missing required fields', () => {
    const result = transformTuistGraph({ name: 'Incomplete' });
    expect(result.data.nodes).toEqual([]);
    expect(result.data.edges).toEqual([]);
