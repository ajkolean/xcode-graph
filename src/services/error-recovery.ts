 ...
 */
export function createLayoutRetryAction(computeLayoutFn: () => Promise<void>): () => Promise<void> {
  return async () => {
    try {
 ...
 */
export function createRefreshDataAction(refreshFn: () => Promise<void>): () => Promise<void> {
  return async () => {
    try {
      await refreshFn();
